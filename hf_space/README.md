---
title: VoiceTut-TTS
emoji: 🎙️
colorFrom: blue
colorTo: indigo
sdk: gradio
sdk_version: "5.49.1"
app_file: app.py
pinned: true
license: apache-2.0
short_description: Egyptian Arabic & code-switching text-to-speech
models:
- mohammedaly22/VoiceTut-TTS
---

# 🎙️ VoiceTut-TTS — Egyptian Arabic TTS

Interactive demo of [VoiceTut-TTS](https://huggingface.co/mohammedaly22/VoiceTut-TTS) — Egyptian Arabic & code-switching text-to-speech, fine-tuned from OmniVoice.

- 17 built-in voices · zero-shot cloning · streaming · text normalization
- Code & notebooks: https://github.com/MohammedAly22/VoiceTuT-TTS
